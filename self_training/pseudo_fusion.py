...
if __name__ == '__main__':
    ...
    # Read test list
    testlist = dtu_test_root+"/scan_list_train.txt"
    ...
    for scan in scans:
        ...
        fusibile_workspace = os.path.join(out_folder, scan)
        ...
        # Move ply file
        consis_folders = [f for f in os.listdir(os.path.join(out_folder,scan)) if f.startswith('consistencyCheck-')]
        consis_folders.sort()
        consis_folder = consis_folders[-1]
        source_ply = os.path.join(out_folder,scan,consis_folder,'final3d_model.ply')
        scan_idx = int(scan[4:])
        target_ply = os.path.join(out_folder,'cvpmvsnet{:03d}_l3.ply'.format(scan_idx))
